...
-- Using a combination of built-in PostgreSQL functions for security
...
CREATE OR REPLACE FUNCTION private.encrypt_token(token TEXT)
RETURNS TEXT AS $$
BEGIN
  ...
  RETURN encode(
    encrypt_iv(
      token::bytea, 
      decode(private.get_encryption_key(), 'hex'),
      gen_random_bytes(16),
      'aes-cbc'
    ), 
    'base64'
...
CREATE OR REPLACE FUNCTION private.decrypt_token(encrypted_token TEXT)
RETURNS TEXT AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION private.create_user_session(
  p_session_id TEXT,
...
BEGIN
  INSERT INTO public.user_sessions (
    ...
    p_session_id,
    private.encrypt_token(p_oauth_token),
    private.encrypt_token(p_oauth_token_secret),
    private.encrypt_token(p_access_token),
    private.encrypt_token(p_access_token_secret),
    p_user_id,
    p_username,
    p_profile_image_url,
    p_status
...
CREATE OR REPLACE FUNCTION private.get_user_session_tokens(p_session_id TEXT)
RETURNS JSON AS $$
DECLARE
  session_data JSON;
BEGIN
  SELECT json_build_object(
    'session_id', session_id,
    'oauth_token', private.decrypt_token(oauth_token),
    'oauth_token_secret', private.decrypt_token(oauth_token_secret),
    'access_token', private.decrypt_token(access_token),
    'access_token_secret', private.decrypt_token(access_token_secret),
    'user_id', user_id,
    'username', username,
    'profile_image_url', profile_image_url,
    'status', status,
...
CREATE OR REPLACE FUNCTION private.update_user_session(
  p_session_id TEXT,
  p_oauth_token TEXT DEFAULT NULL,
  p_oauth_token_secret TEXT DEFAULT NULL,
  p_access_token TEXT DEFAULT NULL,
...
BEGIN
  UPDATE public.user_sessions SET
    oauth_token = COALESCE(private.encrypt_token(p_oauth_token), oauth_token),
    oauth_token_secret = COALESCE(private.encrypt_token(p_oauth_token_secret), oauth_token_secret),
    access_token = COALESCE(private.encrypt_token(p_access_token), access_token),
    access_token_secret = COALESCE(private.encrypt_token(p_access_token_secret), access_token_secret),
...
GRANT EXECUTE ON FUNCTION private.create_user_session TO service_role;
GRANT EXECUTE ON FUNCTION private.get_user_session_tokens TO service_role;
GRANT EXECUTE ON FUNCTION private.update_user_session TO service_role;
